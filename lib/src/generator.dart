...
class FeatureGenerator {
  ...
  Future<void> generateFeatureTests(String name, {bool force = false}) async {
    ...
    try {
      ...
      String? state;
      if (await File('lib/features/$snake/presentation/$stateFolderName/${snake}_bloc.dart').exists()) {
        state = 'bloc';
      } else if (await File('lib/features/$snake/presentation/$stateFolderName/${snake}_provider.dart').exists()) {
        state = 'riverpod';
      }
  ...
  Future<void> _patchAndroidManifest() async {
    ...
    if (!c.contains('INTERNET')) {
      c = c.replaceFirst('<application', '    <uses-permission android:name="android.permission.INTERNET" />\n    <application');
      await file.writeAsString(c);
    }
  }
