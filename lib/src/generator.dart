...
class FeatureGenerator {
  ...
  Future<void> initProject({String? stateManager, bool force = false}) async {
    ...
    try {
      final dirs = [
        ...
        'lib/core/utils', 'lib/features', 'lib/shared/widgets/buttons',
        'lib/shared/widgets/layout', 'lib/shared/state',
      ];
  ...
  Future<void> _patchDI(String featureName, {String? stateManager, String? storageType}) async {
    ...
    if (stateManager == 'bloc') imps.add("import '../../features/$snake/presentation/$stateFolderName/${snake}_bloc.dart';");
    if (stateManager == 'riverpod') imps.add("import '../../features/$snake/presentation/$stateFolderName/${snake}_provider.dart';");

    for (final i in imps) {
       if (!content.contains(i)) content = "$i\n$content";
    }

    final localArgs = (storageType == 'drift' || storageType == 'shared') ? 'sl()' : '';
  ...
  Future<void> generateFeatureTests(String name, {bool force = false}) async {
    ...
    try {
      ...
      if (state == 'bloc') {
        await _createFile('test/features/$snake/presentation/$stateFolderName/${snake}_bloc_test.dart', TestTemplates.blocTest(snake, projectName));
      } else if (state == 'riverpod') {
        await _createFile('test/features/$snake/presentation/$stateFolderName/${snake}_provider_test.dart', TestTemplates.riverpodTest(snake, projectName));
      }
