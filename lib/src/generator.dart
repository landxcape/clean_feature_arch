...
class FeatureGenerator {
  ...
  Future<void> generate(String name,
    ...
    try {
      ...
      final prefsExists = await File('lib/core/storage/local_settings.dart').exists();

      String? storageSelection = (storageType == 'none') ? null : storageType;
      
      if (storageSelection == null && storageType != 'none' && (dbExists || prefsExists)) {
        final options = ['None'];
        if (dbExists) options.add('Drift (SQL)');
        if (prefsExists) options.add('Shared Preferences');
        ...
        if (options.length > 1) {
          ...
          if (choice == 'Drift (SQL)') storageSelection = 'drift';
          if (choice == 'Shared Preferences') storageSelection = 'shared';
        }
      }
      ...
      // 5. Auto-Wiring
      if (storageSelection == 'drift') await _patchDatabase(snakeCaseName);
      await _patchDI(snakeCaseName, stateManager: stateManager, storageType: storageSelection);
  ...
  Future<void> initProject({String? stateManager, bool force = false}) async {
    ...
    try {
      final dirs = [
        'lib/core/config', 'lib/core/constants', 'lib/core/di/modules',
        'lib/core/error', 'lib/core/extensions', 'lib/core/network/interceptors',
        'lib/core/router', 'lib/core/storage', 'lib/core/theme', 'lib/core/types',
        'lib/core/utils', 'lib/core/localization', 'lib/features',
        'lib/shared/widgets/buttons', 'lib/shared/widgets/layout', 'lib/shared/state',
        'assets/images', 'assets/icons', 'assets/fonts', 'assets/animations', 'assets/translations',
      ];
      ...
      await _createFile('lib/core/constants/asset_constants.dart', CoreTemplates.assetConstants());
      await _createFile('lib/core/constants/app_constants.dart', CoreTemplates.appConstants());
      await _createFile('lib/core/localization/string_manager.dart', CoreTemplates.stringManager());
      ...
      await _createFile('lib/core/utils/responsive_utils.dart', CoreTemplates.responsiveUtils());
      ...
      await _createFile('assets/translations/en-US.json', CoreTemplates.emptyJson());
      ...
      progress.complete('Project initialized successfully with Absolute Resource Suite.');
    } catch (e) {
  ...
  Future<void> initStorage(String engine, {bool force = false}) async {
    ...
    try {
      final projectName = _getProjectName();
      if (engine == 'drift') {
        await Process.run('flutter', ['pub', 'add', 'drift', 'sqlite3_flutter_libs', 'path_provider', 'path']);
        await Process.run('flutter', ['pub', 'add', '-d', 'drift_dev']);
        await _createFile('lib/core/storage/app_database.dart', CoreTemplates.driftDatabase(projectName));
        await _patchDIForStorage('drift');
      } else if (engine == 'shared') {
        await Process.run('flutter', ['pub', 'add', 'shared_preferences']);
        await _createFile('lib/core/storage/local_settings.dart', CoreTemplates.localSettings());
        await _patchDIForStorage('shared');
      }
      progress.complete('$engine initialized.');
    } catch (e) { progress.fail('Failed to initialize $engine: $e'); }
  }

  /// Injects storage into existing feature.
  Future<void> addStorageToFeature(String featureName, String engine, {bool force = false}) async {
    _strategy = force ? OverwriteStrategy.always : OverwriteStrategy.ask;
    final progress = _logger.progress('Adding $engine to $featureName');
    try {
      if (engine == 'drift') await _patchDatabase(featureName);
      await _patchFeatureLocalDataSource(featureName, engine);
      await _patchDIForFeatureStorage(featureName, engine);
      progress.complete();
    } catch (e) { progress.fail('Failed: $e'); }
  }
  ...
  Future<void> _patchDIForStorage(String type) async {
    ...
    if (type == 'drift') {
      if (!content.contains('app_database.dart')) content = "import '../storage/app_database.dart';\n$content";
      if (!content.contains('AppDatabase')) {
        content = content.replaceFirst('configureDependencies() async {', 'configureDependencies() async {\n  sl.registerLazySingleton<AppDatabase>(() => AppDatabase());');
      }
    } else if (type == 'shared') {
      if (!content.contains('shared_preferences.dart')) content = "import 'package:shared_preferences/shared_preferences.dart';\nimport '../storage/local_settings.dart';\n$content";
  ...
  Future<void> _patchFeatureLocalDataSource(String featureName, String engine) async {
    final snake = featureName.snakeCase;
    final pascal = featureName.pascalCase;
    final file = File('lib/features/$snake/data/data_sources/local_data_sources/${snake}_local_data_source.dart');
    if (!await file.exists()) return;
    var content = await file.readAsString();
    final projectName = _getProjectName();
  ...
  Future<void> _patchDIForFeatureStorage(String featureName, String engine) async {
    final file = File('lib/core/di/injection_container.dart');
    if (!await file.exists()) return;
    var content = await file.readAsString();
    final pascal = featureName.pascalCase;
  ...
  Future<void> _patchDatabase(String featureName) async {
    ...
    if (match != null && !match.group(1)!.contains('${pascal}Table')) {
      ...
      if (!content.contains(import)) content = "$import\n$content";
      content = content.replaceFirst('tables: [', 'tables: [${pascal}Table, ');
    }
    await file.writeAsString(content);
  }

  Future<void> _patchDI(String featureName, {String? stateManager, String? storageType}) async {
    final file = File('lib/core/di/injection_container.dart');
    if (!await file.exists()) return;
    var content = await file.readAsString();
    final pascal = featureName.pascalCase;
    ...
    if (content.contains('// --- $pascal ---')) return;
    ...
    if (stateManager == 'bloc') imps.add("import 'package:$projectName/features/$snake/presentation/$stateFolderName/${snake}_bloc.dart';");
    if (stateManager == 'riverpod') imps.add("import 'package:$projectName/features/$snake/presentation/$stateFolderName/${snake}_provider.dart';");

    for (final i in imps) if (!content.contains(i)) content = "$i\n$content";
    ...
    if (stateManager == 'bloc') regs += '\n  sl.registerFactory(() => ${pascal}Bloc(sl()));';
    
    content = content.replaceFirst('configureDependencies() async {', 'configureDependencies() async {$regs');
    await file.writeAsString(content);
  }
  ...
  Future<void> _patchPubspecForAssets() async {
    ...
    if (!content.contains('assets:')) {
      final assetsBlock = '\n  assets:\n    - assets/images/\n    - assets/icons/\n    - assets/fonts/\n    - assets/animations/\n    - assets/translations/\n';
      content = content.replaceFirst('uses-material-design: true', 'uses-material-design: true$assetsBlock');
      await file.writeAsString(content);
    }
  }
  ...
  Future<void> addCI(String platform) async {
    ...
    try {
      if (platform == 'github_actions') await _createFile('.github/workflows/verify.yml', CoreTemplates.githubVerify());
      else if (platform == 'gitlab_ci') await _createFile('.gitlab-ci.yml', CoreTemplates.gitlabCI());
      progress.complete();
    } catch (e) { progress.fail('Failed: $e'); }
  }
  ...
  Future<void> initTests() async {
    ...
    try {
      final projectName = _getProjectName();
      await _createFile('test/core/network/api_client_test.dart', CoreTemplates.apiClientTest().replaceFirst('your_project', projectName));
      progress.complete();
    } catch (e) { progress.fail('Failed: $e'); }
  }
  ...
  Future<void> generateFeatureTests(String name, {bool force = false}) async {
    ...
    try {
      ...
      if (await presentationDir.exists()) {
        final subDirs = await presentationDir.list().where((e) => e is Directory).map((e) => p.basename(e.path)).toList();
        if (subDirs.contains('bloc')) stateFolderName = 'bloc';
        else if (subDirs.contains('providers')) stateFolderName = 'providers';
      }
      String? state;
      if (await File('lib/features/$snake/presentation/$stateFolderName/${snake}_bloc.dart').exists()) state = 'bloc';
      else if (await File('lib/features/$snake/presentation/$stateFolderName/${snake}_provider.dart').exists()) state = 'riverpod';
      ...
      if (state == 'bloc') await _createFile('test/features/$snake/presentation/$stateFolderName/${snake}_bloc_test.dart', TestTemplates.blocTest(snake, projectName));
      else if (state == 'riverpod') await _createFile('test/features/$snake/presentation/$stateFolderName/${snake}_provider_test.dart', TestTemplates.riverpodTest(snake, projectName));
      progress.complete();
    } catch (e) { progress.fail('Failed: $e'); }
  }
  ...
  Future<void> addPermission(PermissionMetadata p) async {
    ...
    try {
      ...
      if (p.iosKey.isNotEmpty) await _updateInfoPlist(p.iosKey, p.iosDesc);
  ...
  Future<void> _updateAndroidManifest(List<String> ps) async {
    ...
    if (mod) await file.writeAsString(c);
  }
  ...
  Future<void> _updateInfoPlist(String k, String d) async {
    ...
    if (!await file.exists()) return;
    var c = await file.readAsString();
    if (!c.contains(k)) { c = c.replaceFirst('<dict>', '<dict>\n\t<key>$k</key>\n\t<string>$d</string>'); await file.writeAsString(c); }
  }
  ...
  Future<void> _updatePermissionService(String name) async {
    ...
    if (!c.contains('request$p()')) c = c.replaceFirst('openSettings();', 'openSettings();\n  Future<bool> request$p();');
  ...
  Future<void> _addDependencies({String? stateManager}) async {
    ...
    try {
      final deps = ['get_it', 'go_router', 'dio', 'fpdart', 'freezed_annotation', 'json_annotation', 'retrofit', 'flutter_secure_storage', 'internet_connection_checker_plus', 'permission_handler', 'logger', 'pretty_dio_logger', 'easy_localization'];
      if (stateManager == 'bloc') deps.add('flutter_bloc');
      else if (stateManager == 'riverpod') { deps.add('flutter_riverpod'); deps.add('riverpod_annotation'); }
      final devDeps = ['freezed', 'json_serializable', 'build_runner', 'retrofit_generator', 'mocktail', 'clean_feature_arch'];
      if (stateManager == 'riverpod') devDeps.add('riverpod_generator');
  ...
  Future<void> _patchAndroidManifest() async {
    ...
    if (!await file.exists()) return;
    var c = await file.readAsString();
    if (!c.contains('INTERNET')) { c = c.replaceFirst('<application', '    <uses-permission android:name="android.permission.INTERNET" />\n    <application'); await file.writeAsString(c); }
  }
  ...
  Future<void> _patchInfoPlist() async {
    ...
    if (!await file.exists()) return;
    var c = await file.readAsString();
    final ps = {'NSCameraUsageDescription': 'Needs camera.', 'NSLocationWhenInUseUsageDescription': 'Needs location.', 'NSPhotoLibraryUsageDescription': 'Needs photos.'};
    bool mod = false;
    for (final e in ps.entries) if (!c.contains(e.key)) { c = c.replaceFirst('<dict>', '<dict>\n\t<key>${e.key}</key>\n\t<string>${e.value}</string>'); mod = true; }
    if (mod) await file.writeAsString(c);
  }
  ...
  Future<void> _createFile(String path, String content) async {
    ...
    if (await file.exists()) {
      if (_strategy == OverwriteStrategy.skipAll) return;
      if (_strategy == OverwriteStrategy.ask) {
        final c = _logger.chooseOne('File $path exists. Overwrite?', choices: ['Yes', 'No', 'Always', 'Skip All'], defaultValue: 'No');
        if (c == 'Always') _strategy = OverwriteStrategy.always;
        else if (c == 'Skip All') { _strategy = OverwriteStrategy.skipAll; return; }
        else if (c == 'No') return;
      }
    }
    await file.writeAsString(content);
