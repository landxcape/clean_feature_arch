...
class FeatureGenerator {
  ...
  Future<void> generate(String name, {String? targetDirectory}) async {
    ...
    try {
      ...
      await _createFile(
        p.join(baseDir, 'domain', 'repositories', '${snakeCaseName}_repository.dart'),
      ...
      // 2. Data Layer
      await _createFile(
        p.join(baseDir, 'data', 'models', 'requests', '${snakeCaseName}_request_model.dart'),
        DataTemplates.requestModel(snakeCaseName),
      );
      await _createFile(
        p.join(baseDir, 'data', 'models', 'responses', '${snakeCaseName}_response_model.dart'),
        DataTemplates.responseModel(snakeCaseName),
      );
      await _createFile(
        p.join(baseDir, 'data', 'models', 'local', '${snakeCaseName}_local_model.dart'),
        DataTemplates.localModel(snakeCaseName),
      );
      await _createFile(
        p.join(baseDir, 'data', 'datasources', '${snakeCaseName}_remote_datasource.dart'),
        DataTemplates.remoteDatasource(snakeCaseName),
      );
      await _createFile(
        p.join(baseDir, 'data', 'repositories', '${snakeCaseName}_repository_impl.dart'),
  ...
  Future<void> initProject() async {
    ...
    try {
      ...
      await _createFile('lib/core/error/app_error.dart', CoreTemplates.appError(), overwrite: true);
      await _createFile('lib/core/error/error_handler.dart', CoreTemplates.errorHandler(), overwrite: true);
      await _createFile('lib/core/di/injection_container.dart', CoreTemplates.injectionContainer(), overwrite: true);
      await _createFile('lib/core/network/api_client.dart', CoreTemplates.apiClient(), overwrite: true);
      await _createFile('lib/core/types/typedefs.dart', CoreTemplates.typedefs(), overwrite: true);
      await _createFile('lib/core/utils/logger.dart', CoreTemplates.logger(), overwrite: true);
      
      // 3. Generate Root Files
      await _createFile('lib/main.dart', CoreTemplates.mainDart(), overwrite: true);
      await _createFile('lib/app.dart', CoreTemplates.appDart(), overwrite: true);
      await _createFile('analysis_options.yaml', CoreTemplates.analysisOptions(), overwrite: true);
  ...
  Future<void> _addDependencies() async {
    ...
    try {
      ...
      if (depResult.exitCode != 0) {
        _logger.warn('Note: flutter pub add failed. Addition may be required manually.');
      }

      _logger.detail('Running: flutter pub add -d ${devDeps.join(' ')}');
      final devDepResult = await Process.run('flutter', ['pub', 'add', '-d', ...devDeps]);
      if (devDepResult.exitCode != 0) {
        _logger.warn('Note: flutter pub add -d failed. Addition may be required manually.');
      }
  ...
  Future<void> _createFile(String path, String content, {bool overwrite = false}) async {
